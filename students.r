...
citizenships <- select(citizenships, all_of(pk_for_citizenships), everything()) 
# a data frame for disabilities
disabilities <- filter_columns(students_data, disabilities_data, pk_for_students)
disabilities <- select(disabilities, all_of(pk_for_disabilities), everything())
...
address <- select(address, "AddressKey", everything())
...
students <- students %>%
    ...
    select(pk_for_students,everything())
...
# write the data
write.csv(citizenships, file = file.path(getwd(), "Citizenships.csv"), row.names = FALSE)
write.csv(disabilities, file = file.path(getwd(), "Disabilities.csv"), row.names = FALSE)
write.csv(students_and_subclass, file = file.path(getwd(),"StudentsAndSubclass.csv"), row.names = FALSE)
write.csv(students, file = file.path(getwd(), "Students.csv"), row.names = FALSE)
write.csv(address, file = file.path(getwd(), "Address.csv"), row.names = FALSE)
